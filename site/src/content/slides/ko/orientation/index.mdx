...
### 새 주제 받기 · VS Code로 열기
::sub[로컬에서도 Codespaces와 같은 환경입니다]

- 실행

```sh
git pull        # 새 주제가 올라오면 받는다
code .          # VS Code로 이 폴더를 연다
```

VS Code가 **Reopen in Container**를 물어보면 누릅니다.  
안 뜨면 명령 팔레트에서 **Dev Containers: Reopen in Container**

<br/>

같은 `.devcontainer` 설정을 쓰므로 컴파일러·Python·확장이 그대로 준비됩니다.  
▶ 버튼과 `F5`도 Codespaces에서와 똑같이 동작합니다.
...
<GradeChart size="clamp(120px, 16vw, 180px)" items={[
  { label: '중간시험', value: 20, color: 'var(--aas-role-model)' },
  { label: '기말시험', value: 40, color: 'var(--aas-role-eval)' },
  { label: '과제1', value: 5, color: 'var(--aas-role-trace)' },
  { label: '과제2', value: 5, color: 'var(--aas-role-source)' },
  { label: '개인프로젝트', value: 25, color: 'var(--aas-role-tool)' },
  { label: '출석', value: 5, color: 'var(--aas-role-guard)' },
]} />
...
- 강의 자료는 다음 언어로 제공됩니다.
  ...
  - 영어
  - 중국어 간체
- 추가 언어가 필요하면 tkl@yonsei.ac.kr로 메일 보내 주세요.
- 과제·개인프로젝트·시험 답안은 다음 중 하나의 언어로 작성해야 합니다.
  - 한국어
  - 영어
...
- 시수는 수업 시간 단위입니다. 1시간 수업은 시수 1, 2시간 수업은 시수 2입니다.
  - 수요일 5교시(1시수) + 금요일 5·6교시(2시수) = 주당 3시수
- 학기 전체 결석 가능 시수는 **총 6시수**입니다.
- 6시수를 넘으면 초과 시수마다 출석 점수에서 **0.25점**씩 감점됩니다.
- 출결에서 자유롭고 싶다면 **15분 발표**로 출석 만점을 받을 수 있습니다.
  주제는 컴퓨터 관련이면 되고, AI 관련 주제도 좋습니다.
- LearnUs 출석 인식이 안 되면 강의실에서 찍은 사진을 tkl@yonsei.ac.kr로 보내
  주세요.
...
1. **GitHub 계정**을 만들고, `algorithm-env`에서 본인 저장소를 만듭니다.
2. **Codespace**에서 `algorithm-code` 수업 예제를 돌려 봅니다.
