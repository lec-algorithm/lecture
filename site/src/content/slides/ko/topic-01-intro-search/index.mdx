...
{/* 슬라이드 작성 규칙은 docs/writing-rules.md의 "슬라이드 규칙"을 따릅니다.
    예제마다 절차 → 코드 → 실행 → 실습 순서로 갑니다. 코드는 저장소 파일을
    그대로 옮긴 것이므로, 저장소가 바뀌면 여기도 함께 고칩니다. */}
...
```sh
cd src/topic-01-intro-search/02_binary_search
make -f /work/Makefile binarySearch.out && ./binarySearch.out
```

1. `key`를 첫 값 `6`, 마지막 값 `101`로 바꿔 비교 횟수를 봅니다.
2. **이진 검색의 배열을 뒤섞어 보세요.** 답이 틀립니다. 순차 검색은 그대로 맞습니다.
...
```sh
cd src/topic-01-intro-search/03_fibonacci
make -f /work/Makefile fibonacci.out && ./fibonacci.out
```

1. `n`을 45, 50으로 올려 보세요. 재귀가 언제 실용성을 잃는지 보입니다.
2. Python은 `n = 35`에서 멈춰 둡니다. 같은 코드가 훨씬 느립니다.
   **어디서 멈추는지도 관찰의 일부입니다.**
