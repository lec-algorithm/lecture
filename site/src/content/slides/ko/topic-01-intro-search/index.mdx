...
import SubtractionDemo from '@components/SubtractionDemo.astro';
...
오른쪽은 아직 답이 아닙니다.  
음수 자리를 없애려면 오른쪽부터 10을 빌려 올려야 합니다.

$$
(2)\,(-3)\,(-1) \;\longrightarrow\; (2)\,(-4)\,(9) \;\longrightarrow\; (1)\,(6)\,(9) = 169
$$

**이 마무리는 순차적입니다.** 오른쪽을 정리해야 왼쪽 값이 정해집니다.  
즉 **계산은 병렬로, 정리는 순차로** 하는 구조입니다.
...
### 같은 답, 다른 연산 방식
::sub[한 단계에 몇 자리가 동시에 일하는지 보세요]

<SubtractionDemo slide a={361} b={192} />

왼쪽은 앞 자리의 빌림이 정해져야 다음 자리를 시작합니다.  
오른쪽은 세 자리를 한 단계에 끝내고, 정리를 뒤로 미룹니다.
...
| --- | --- | --- |
| 자리별 계산 | 순차적 | **병렬 가능** |
| 자리 사이 의존 | 있음 | 뺄 때 없음, 정리할 때 있음 |
| 마무리 | 없음 | 자리올림 전파 |

자리를 동시에 처리할 하드웨어가 있다면 오른쪽,  
한 번에 하나씩 처리한다면 뒤처리가 없는 왼쪽입니다.
