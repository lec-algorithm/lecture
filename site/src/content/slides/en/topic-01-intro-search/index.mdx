...
The right one is not the answer yet.  
To clear the negative digits, carry a 10 leftwards starting from the right.

$$
(2)\,(-3)\,(-1) \;\longrightarrow\; (2)\,(-4)\,(9) \;\longrightarrow\; (1)\,(6)\,(9) = 169
$$

**This tidying is sequential.** The left digit settles only after the right one.  
So the shape is **compute in parallel, tidy up in sequence**.
...
::sub[Watch how many columns work in each step]

<SubtractionDemo slide a={361} b={192} />

On the left, a column cannot start until the borrow beside it is known.  
On the right, all three finish in one step and the tidying is deferred.
...
| --- | --- | --- |
| Per-column work | sequential | **can be parallel** |
| Dependency between columns | yes | none while subtracting, yes while tidying |
| Finishing | none | carry propagation |

With hardware that handles the columns at once, the right one wins.  
Handling one column at a time, the left one wins: no cleanup pass.

Both give the right answer.  
**Where you are going to run it** decides.
...
::sub[Every comparison halves the range that is left]
