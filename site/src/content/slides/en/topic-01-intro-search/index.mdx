...
| Average | $$\dfrac{n+1}{2}$$ | $$O(n)$$ | **the value is in the array** and evenly distributed |
| Worst | $$n$$ | $$O(n)$$ | it is the last element, or not there at all |

A value that is not there always costs all n, which is why the average carries
a condition.  
**When you state a performance, state which instances you assume.**

**Space** is $$O(1)$$ in all three: one index variable.
...
### Binary search
::sub[Look at the middle, throw away the half you no longer need. Press **step**]

<SearchDemo slide mode="binary" values={[6, 13, 14, 25, 33, 43, 51, 53, 64, 72, 84, 93, 95, 96, 101]} target={51} />

Every comparison halves the candidates that are left.  
...
**Three.** Measuring all fifteen positions gives an average of $$49/15 \approx 3.3$$.

</Slide>

<Slide class="compact">

#### Worst: splitting all the way down
::sub[`key = 51` · the four steps laid out at once]
...
#### How many comparisons
::sub[Binary search costs between 1 and 4, depending on where the key sits]

| | Comparisons | Time | When |
| --- | --- | --- | --- |
| Best | $$1$$ | $$O(1)$$ | the middle is the `key` |
| Average | about $$\log_2 n$$ | $$O(\log n)$$ | wherever it sits |
| Worst | $$\lfloor \log_2 n \rfloor + 1$$ | $$O(\log n)$$ | the deepest cell, or not there at all |

For fifteen elements: 1 · about 3.3 · 4.  
Set that against sequential search's 1 · 8 · 15.

**Space** is $$O(1)$$ in all three: just `lo`, `hi` and `mid`.
...
Ten more in n multiplies the calls by **roughly 100**, and the time with it.  
Iteration stays around 0.001 ms while n quadruples.  
