...
### 获取新主题 · 用 VS Code 打开
::sub[在本地也是与 Codespaces 相同的环境]

- 执行

```sh
git pull        # 有新主题时获取
code .          # 用 VS Code 打开这个文件夹
```

VS Code 询问 **Reopen in Container** 时点击它。  
若没有提示，用命令面板：**Dev Containers: Reopen in Container**

<br/>

因为读取同一份 `.devcontainer` 配置，编译器、Python 与扩展都已就绪。  
▶ 按钮和 `F5` 的行为与 Codespaces 中完全一致。
...
<GradeChart size="clamp(120px, 16vw, 180px)" items={[
  { label: '期中考试', value: 20, color: 'var(--aas-role-model)' },
  { label: '期末考试', value: 40, color: 'var(--aas-role-eval)' },
  { label: '作业 1', value: 5, color: 'var(--aas-role-trace)' },
  { label: '作业 2', value: 5, color: 'var(--aas-role-source)' },
  { label: '个人项目', value: 25, color: 'var(--aas-role-tool)' },
  { label: '出勤', value: 5, color: 'var(--aas-role-guard)' },
]} />
...
- 讲义提供以下语言：
  ...
  - 英语
  - 简体中文
- 需要其他语言支持，请发邮件至 tkl@yonsei.ac.kr。
- 作业、个人项目与考试答案须用以下语言之一书写：
  - 韩语
  - 英语
...
- 课时是出勤的计量单位：1 小时的课算 1 课时，2 小时的课算 2 课时。
  - 周三第 5 节（1 课时）+ 周五第 5\~6 节（2 课时）= 每周 3 课时
- 整个学期可缺勤的课时总计**不超过 6 课时**。
- 超过之后，每多缺勤 1 课时，出勤分数扣 **0.25 分**。
- 不想被出勤约束的话，做一次**15 分钟的演讲**即可拿到出勤满分。主题只要与
  计算机相关即可，AI 相关话题也可以。
- 如果 LearnUs 无法识别出勤，请把教室拍照发送至 tkl@yonsei.ac.kr。
...
1. 注册 **GitHub 账号**，并从 `algorithm-env` 创建自己的仓库。
2. 在 `algorithm-code` 上创建 **Codespace**，运行课堂示例。
