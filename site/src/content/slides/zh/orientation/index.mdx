...
### 创建自己的仓库 ①
::sub[在自己的副本中工作，而不是公共仓库]

<Screenshot src={repoTemplate} alt="algorithm-code 仓库上的 Use this template 按钮，以及展开的 Create a new repository 菜单" maxHeight="40vh" />

`algorithm-code` 是 **template 仓库**。  
...
<Screenshot src={repoCreate} alt="Create a new repository 界面，仓库名填写 my-algorithm-code，下方是 Create repository 按钮" />

取一个容易辨认的名字，例如 `my-algorithm-code`。  
...
### 在自己的仓库中打开 Codespaces
::sub[无需安装任何东西]

1. 打开**刚创建的自己的仓库**。
2. 绿色的 **Code** 按钮 → **Codespaces** 标签页
