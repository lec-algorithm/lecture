...
### 划分也可以同时做 \{#parallel-quick}

归并排序的并行故事，快排也有。**同一层的划分碰的是不同的区间**，
所以可以同时做。只是方向翻了过来：归并从下往上合，快排从上往下切。

<ParallelQuickDemo />

工作量还是六次划分，一层一起做只要 **3 步**，也就是树的深度。
方向翻了，墙钟的下限也翻了：归并是**最后一次归并**独自处理整个
数组，快排是**第一次划分**。

随机基准的又一个好处在这里露面。树变成独木道（比如已排序输入）时，
每层只有一个划分，**并行收益归零**。好基准不只省时间，
还**造出可以并行的空间。**
...
- 也试试最好情况的数组 `5 1 3 4 2 8 7 6 9 10`：19 次，这个规模的
  最小值。
