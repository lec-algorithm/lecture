...
Step through the same subtraction both ways. What to watch is not the result
but **how many columns work in each step**.

<SubtractionDemo a={361} b={192} />

On the left a column cannot start until the borrow beside it is known, so each
step handles exactly one. On the right all three finish in a single step,
because none of them has any reason to wait.

At three digits the step counts tie, three against three, because the parallel
subtraction is followed by a sequential tidy. **At n digits the left-hand
subtraction takes n steps and the right-hand one always takes 1.** Chain
several operations together and tidy up only once at the end, and that
difference is what remains. Deferring the carry this way is exactly what a
hardware multiplier does (carry-save).
...
| Average | $$\dfrac{n+1}{2}$$ | **the value is in the array** and its position is evenly distributed |
...
Taking the worst case: **time** $$O(n)$$, and since the only extra memory is an
index variable, **space** $$O(1)$$.
...
Every ten steps of n multiply the call count by **roughly 100**. Iteration grows
in proportion to n, which rounds to zero in this table.
