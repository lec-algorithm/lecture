...
- **개인프로젝트 안내를 추가했다.** 주제는 각자 고르고, 같은 문제를 푸는
  기존 방식 셋 이상을 직접 돌려 비교하는 과제다. 주제의 조건과 비교 기준,
  중간 제출(슬라이드, 10월 18일)과 최종 제출(보고서·코드, 12월 13일),
  평가에서 보는 것, 지켜야 할 것을 글 하나에 정리했다.
- 같은 내용을 수업에서 띄울 **슬라이드 17장**으로도 만들었다. 주제 예시는
  문자열·압축·경로 같은 컴퓨터 쪽에 더해 통신·신호·나노·생물까지 넓혔다.
- 글과 덱 모두 한국어·영어·중국어로 만들었다.
