...
- 여기는 **강의 자료만** 담는다. 실행되는 코드는 조직
  ...
  - `algorithm-env`: 실습 환경 template. 수강생이 `Use this template`으로 과제·
    개인프로젝트용 저장소를 만든다. 환경 파일(compose·Dockerfile·Makefile·
    devcontainer·`.vscode`)의 **단일 원본**이다.
  - `algorithm-code`: 주제별 강의 예제. 학기 중에 자란다. 수강생은 **복사하지
    않고** 저장소에서 바로 codespace를 만들거나 클론해서 `git pull`로 받는다.
...
- 중국어 문자열은 `site/src/data/site.ts`가 공급한다. `name`(로케일별 사이트
  이름) · `ui.zh` · `pricingLabels.zh` · `difficultyLabels.zh` 넷이다. 빠진 것은
  한국어로 떨어지므로, 빌드 후 `python3 scripts/check-locale-leaks.py
  site/dist/zh`로 확인한다.
...
- **코드 블록 두 개를 붙여 놓지 않는다.** 명령과 출력을 나란히 보일 때는
  각 펜스 앞에 `- 실행` · `- 결과` 라벨을 둔다. 다른 관계면 파일명 등
  내용에 맞는 라벨을 쓴다.
